...
module.exports = function(program) {
	...
	if (program.liveReload) {
	  ...
	  gaze.on('all', function(event, filePath) {
		...
		localhost.use('/livereload', bodyParser.json(), tinylr.middleware({
			app: localhost,
			port: program.port
		}));
	}
	...
	// Perform on the fly pre-processing by shelling out to the specified CLI tool.
	localhost.get('/preproc/:tool/*', function(req, res, next) {
		var filePath = req.params[0];
		var contentType;

		// Check if the file has two extensions.
		var pathParts = filePath.split('.');
		if (pathParts.length > 2) {
			// chop off the last part
			contentType = mime.lookup(_.last(pathParts));
			filePath = pathParts.slice(-1).join('.');
		}
		else
			contentType = mime.lookup(filePath);

		// Use path.join to normalize for OS slash characters.
		filePath = path.resolve(program.baseDir, path.join(filePath.split('/')));

		// TODO: Use etags to check if the file has changed or not. If not
		// simply return a 304 not modified.

		// Pipe the file through the CLI tool
		fs.createReadStream(filePath)
			.on('error', function(err) {
				// catch file does not exist.
				return next(Error.http(404, "File not found"));
			})
			.pipe(childProcess.spawn(executable, [], {stdio: 'pipe', env: process.env})
			.pipe(res);
	...
	localhost.get(/sandbox\/(.*)$/ , function(req, res, next) {
		log.info("sandbox request for %s with hash %s", req.params[0], req.query.hash);
    ...
    if (!returnUrl.host)
      ...
      // TODO: Pipe the stream through htmlprep to expand src paths
			// but that is the only step that should execute. Need to
			// have extra options to pass into htmlprep
  ...
  return localhost;
	...
	function uploadSandboxFile(pagePath, fileContents, hash, callback) {
		...
		log.debug("uploading file %s to %s with hash %s",
