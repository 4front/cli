    ...
    if (!returnUrl.host)
      return next(Error.http(400, "No valid URL in the return query"));
		...
		fs.exists(fullPath, function(exists) {
			// If the file doesn't exist, invoke the /notfound endpoint of the API
			// and redirect as usual. This will allow the server to render a custom
			// 404 error if so configured.
			if (!exists) {
				return api(program, {
					method: 'post',
					path: '/dev/' + program.virtualApp.appId + '/notfound/' + pagePath
				}, function(err) {
			...
			uploadFileToServer(pagePath, fullPath, req.query, function(err) {
				if (err) return next(err);
