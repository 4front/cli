...
module.exports = function(program) {
	...
	localhost.use(express.static(program.cwd, {
		index: false
	}));
	...
	localhost.get(/sandbox\/(.*)$/ , function(req, res, next) {
		log.info("sandbox request for %s", req.params[0]);
    ...
    if (!returnUrl.host)
      return next(Error.http(400, "No valid URL in the return query"));
		...
		var fullPath = path.join(program.cwd, pagePath);
        ...
        .on('finish', function() {
          ...
          if (fileHash === req.query.hash)
            return res.redirect(req.query.return);
					...
					log.debug("uploading file %s", pagePath);
