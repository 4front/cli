...
module.exports = function(program, callback) {
	...
	seriesTasks.push(function(cb) {
		if (program.profile) {
			selectedProfile = _.find(configJson.profiles, {name: program.profile});
		}

		// If no selectedProfile, try finding the default one.
		if (!selectedProfile)
			selectedProfile = _.find(configJson.profiles, {default: true}) || configJson.profiles[0];
	...
	seriesTasks.push(function(cb) {
		...
		program.inquirer.prompt([{
			...
			program.api.login(answers.username, answers.password, function(err, jwt) {
				if (err) return cb(err);
	...
	async.series(seriesTasks, function(err) {
		...
		fs.writeFile(program.configFilePath, JSON.stringify(configJson, null, 2), function(err) {
			if (err) return callback(err);
