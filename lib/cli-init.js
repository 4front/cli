...
module.exports = function(program, options, callback) {
  ...
  if (options.loadManifest && options.loadVirtualApp) {
  ...
  function selectProfile(cb) {
    ...
    var profileUrl = parseUrl(program.profile.url);
    program.virtualHost = profileUrl.host;
  ...
  function loadManifest(cb) {
    ...
    manifest.load(program, function(err, config) {
      ...
      if (program.appId)
        program.virtualAppManifest.appId = program.appId;
  ...
  function loadVirtualApp(cb) {
    if (!program.virtualAppManifest)
      return cb();

    log.debug("invoking api to fetch the virtual app");
    api(program, {method: 'GET', path: '/apps/' + program.virtualAppManifest.appId}, function(err, app) {
      if (err) return cb(err);
      if (!app)
