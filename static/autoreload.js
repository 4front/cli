  ...
  function ajaxGet(settings) {
    ...
    xhr.onreadystatechange = function() {
      ...
      if (xhr.responseText.length === 0) {
        // This fires right after a page refresh.. we only care if
        // polling has been underway for a few seconds.
        if (Date.now() - pollingStart > 10 * 1000) {
          settings.error({message: 'empty response', timeout: true});
        }
